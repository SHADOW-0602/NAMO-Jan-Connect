...
function apiUrl(path: string) { return `${typeof globalThis !== "undefined" && (globalThis as typeof globalThis & { __NJC_API_URL__?: string }).__NJC_API_URL__ || ""}${path}`; }
function fetch(input: RequestInfo | URL, init?: RequestInit) { const target = typeof input === "string" && input.startsWith("/api") ? apiUrl(input) : input; return globalThis.fetch(target, init); }
...
export default function NamoApp({ initialPortal = "public" }: { initialPortal?: Portal }) {
  ...
  useEffect(() => {
    fetch(apiUrl("/api/complaints?scope=stats")).then((response) => response.ok ? response.json() : Promise.reject()).then((data) => setStats({ total: Number(data.summary.total), resolved: Number(data.summary.resolved), active: Number(data.summary.active), avgDays: Number(data.summary.avgDays ?? 0) })).catch(() => {});
  }, []);
  ...
  async function loadPortal(next: Portal) {
    ...
    try {
      const response = await fetch(apiUrl(`/api/complaints?scope=${scope}`), { headers: demoHeaders(next) });
...
function AdminPortal({ complaints, onSelect, stats, departmentAccess, onChanged }: { complaints: Complaint[]; onSelect: (complaint: Complaint) => void; stats: { total: number; resolved: number; active: number; avgDays: number }; departmentAccess: DepartmentAccess[]; onChanged: () => void }) {
  return <div className="portal-shell"><Sidebar portal="admin" /><main className="workspace"><div className="workspace-heading"><div><p className="eyebrow">PLATFORM OVERSIGHT</p><h1>Administrator control centre</h1><p>See every citizen report, its destination, current owner, and status.</p></div></div><div className="metric-row"><Metric label="All complaints" value={stats.total.toLocaleString("en-IN")} note="Platform lifetime" /><Metric label="Active workload" value={stats.active.toLocaleString("en-IN")} note="Across 5 departments" tone="blue" /><Metric label="Departments" value={departmentAccess.length || 5} note="Isolated staff queues" /><Metric label="Resolution rate" value={`${stats.total ? Math.round((stats.resolved / stats.total) * 100) : 0}%`} note="All departments" tone="green" /></div><DepartmentAccessPanel entries={departmentAccess} onChanged={onChanged} /><section className="workspace-section"><div className="table-heading"><div><h2>All complaints</h2><p>Citizen, department, and live status in one auditable view.</p></div></div><div className="data-table"><div className="table-row table-labels"><span>Complaint / citizen</span><span>Department</span><span>Status</span><span>Filed</span><span></span></div>{complaints.map((complaint) => <div className="table-row" key={complaint.id}><button className="table-complaint" onClick={() => onSelect(complaint)}><CategoryBadge category={complaint.category} /><span><b>{complaint.title}</b><small>{complaint.citizenName ?? "Citizen"} · {complaint.citizenEmail ?? complaint.trackingId}</small></span></button><span>{complaint.department}</span><StatusPill status={complaint.status} /><span>{formatDate(complaint.createdAt)}</span><button className="row-action" onClick={() => onSelect(complaint)}>View</button></div>)}</div></section></main></div>;
}

function DepartmentAccessPanel({ entries, onChanged }: { entries: DepartmentAccess[]; onChanged: () => void }) {
  const [saving, setSaving] = useState<number | null>(null); const [message, setMessage] = useState("");
  async function assign(entry: DepartmentAccess, staffEmail: string, password: string) { setSaving(entry.departmentId); setMessage(""); try { const response = await fetch("/api/complaints", { method: "PATCH", headers: { "content-type": "application/json", ...demoHeaders("admin") }, body: JSON.stringify({ action: "assign_department", departmentId: entry.departmentId, staffEmail, password }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); setMessage(`${entry.department} credentials updated.`); onChanged(); } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Could not update access"); } finally { setSaving(null); } }
  return <section className="workspace-section department-access"><div className="table-heading"><div><h2>Department IDs & access</h2><p>Assign a staff email and a unique password to every portal.</p></div>{message && <span className="access-message">{message}</span>}</div><div className="department-account-grid">{entries.map((entry) => <form key={entry.departmentId} onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); void assign(entry, String(data.get("staffEmail") ?? ""), String(data.get("password") ?? "")); event.currentTarget.reset(); }}><CategoryBadge category={entry.category} full /><code>{entry.portalId}</code><label>Authorized staff email<input name="staffEmail" type="email" defaultValue={entry.staffEmail ?? ""} placeholder="officer@department.gov.in" required /></label><label>{entry.passwordConfigured ? "Set a new portal password" : "Portal password"}<input name="password" type="password" minLength={12} autoComplete="new-password" placeholder="At least 12 characters" required /></label><a href={`/${entry.category === "civic_infra" ? "civic-infra" : entry.category === "health_edu" ? "health-education" : entry.category === "law_order" ? "law-order" : entry.category === "employment_welfare" ? "employment-welfare" : "transport"}`}>Open separate URL</a><button className="btn btn-small" disabled={saving === entry.departmentId}>{saving === entry.departmentId ? "Saving..." : entry.passwordConfigured ? "Update credentials" : "Assign access"}</button></form>)}</div></section>;
}
...
function UpdateModal({ complaint, onClose, onChanged }: { complaint: Complaint; onClose: () => void; onChanged: () => void }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setBusy(true); setError(""); try { const form = new FormData(); form.set("complaintId", String(complaint.id)); form.set("status", status); form.set("remarks", remarks); if (photo) form.set("resolutionPhoto", photo); const response = await fetch("/api/complaints", { method: "PATCH", headers: demoHeaders("department"), body: form }); const data = await response.json(); if (!response.ok) throw new Error(data.error); onChanged(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Update failed"); } finally { setBusy(false); } }
